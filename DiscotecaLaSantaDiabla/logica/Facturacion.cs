...
namespace DiscotecaLaSantaDiabla.logica
{
    class Facturacion
    {
        public class CreaTicket
        {
            ...
            {
                string LineEncavesado = "Bebida              Cant    P.Unit     Valor";   // agrega lineas de  encabezados
                line.AppendLine(LineEncavesado);
            }

            public static void bebida(String nombre, int cant, double subtotal, double total)
            {
                string LineEncavesado = nombre + "       " + cant + "   " + subtotal + "    " + total;   // agrega lineas de  encabezados
                line.AppendLine(LineEncavesado);
            }
        ...
        public class RawPrinterHelper
        {
            ...
            {
                int factura = BaseDeDatosFactura.obtenerValorUltimaFactura();
                Int32 dwError = 0, dwWritten = 0;
                IntPtr hPrinter = new IntPtr(0);
                DOCINFOA di = new DOCINFOA();
                bool bSuccess = false; // Assume failure unless you specifically succeed.

                di.pDocName = "Factura - " + DateTime.Now.ToLongDateString() ;
                di.pDataType = "RAW";
                //di.pOutputFile = @"D:\Universidad\6 Semestre\Ingenieria de Software Parte 2\La Santa Diabla\DiscotecaLaSantaDiabla\DiscotecaLaSantaDiabla\bin\Debug\netcoreapp3.1\FacturasGeneradas";
        ...
        {
            Facturacion.CreaTicket Ticket1 = new Facturacion.CreaTicket();

            Ticket1.TextoCentro("La Santa Diabla"); //imprime una linea de descripcion
            Ticket1.TextoCentro("**********************************");

            Ticket1.TextoIzquierda("Identifiacion: " + factura.getIdUsuario());
            Ticket1.TextoIzquierda("Nombre: " + factura.getNombreUsuario());
            Ticket1.TextoIzquierda("Apellido: " + factura.getApellidoUsuario());
            Ticket1.TextoIzquierda("Telefono: " + factura.getTelefonoUsuario());
            Ticket1.TextoIzquierda("");
            Ticket1.TextoCentro("**********************************");
            Ticket1.TextoCentro("Factura de Venta"); //imprime una linea de descripcion
            Ticket1.TextoIzquierda("Id Fac: " + factura.getIdFactura());
            Ticket1.TextoIzquierda("Fecha: " + factura.getFechaFactura());
            Ticket1.TextoIzquierda("");
            Facturacion.CreaTicket.LineasGuion();

            Facturacion.CreaTicket.EncabezadoVenta();
            Facturacion.CreaTicket.LineasGuion();

            double unitario = factura.getPrecioTotal() / factura.getCantidadBebida();

            Facturacion.CreaTicket.bebida(factura.getNombreBebida(), factura.getCantidadBebida(), factura.getPrecioSubtotal(), factura.getPrecioTotal());

            Facturacion.CreaTicket.LineasGuion();
            Ticket1.AgregaTotales("Sub-Total", factura.getPrecioSubtotal()); // imprime linea con Subtotal
            Ticket1.AgregaTotales("Total", factura.getPrecioTotal()); // imprime linea con total

            // Ticket1.LineasTotales(); // imprime linea 

            Ticket1.TextoCentro("**********************************");
            Ticket1.TextoCentro("* Vuelva Pronto, La Santa Diabla *");
